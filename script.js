...
// REPLACE THESE WITH YOUR OWN KEYS FROM SUPABASE
const SUPABASE_URL = 'https://supabase.com/dashboard/project/xugasmrxombmdgnukfky/settings/api-keys/legacy';
...
async function addTeam() {
    ...
    else { input.value = ''; await fetchTeams(); render(); }
}
...
async function addPlayer() {
    ...
    const { error } = await supabase.from('players').insert({
        ...
        goals: 0, saves: 0, defense: 0, snitches: 0
    });

    if (error) { alert("Error: " + error.message); } 
    else { document.getElementById('playerNameInput').value = ''; await fetchPlayers(); render(); }
}
...
async function updateStat(type) {
    ...
    if(!error) { await fetchPlayers(); render(); }
}
...
async function addMatch() {
    ...
    // 2. Update Players (Snitch & Defense)
    if (snitchId) {
...
function render() {
    ...
    // Standings
    document.getElementById('standingsBody').innerHTML = getStandings().map((t, i) => `
    ...
    document.getElementById('playerTableBody').innerHTML = ranked.map(p => `
        <tr>
    ...
    // History
    document.getElementById('matchHistory').innerHTML = matches.slice().reverse().map(m => 
    ...
    if (ranked.length > 0) {
        ...
        const calcP = (n, max) => Math.min((n / max) * 100, 100);
        document.getElementById('goalCircle').style.setProperty('--p', calcP(top.goals, 20));
        document.getElementById('saveCircle').style.setProperty('--p', calcP(top.saves, 15));
    }
}
...
function clearData() {
    alert("To reset data, you must delete rows in your Supabase dashboard.");
}
